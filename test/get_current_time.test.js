...
test('get_current_time returns time in HH:MM:SS format', () => {
  ...
  assert.ok(timePattern.test(result.timeAndDate), 'Time should match HH:MM:SS format')
})

test('get_current_time returns date in YYYY/MM/DD format', () => {
  const result = getCurrentTime()
  const datePattern = /\d{4}\/\d{2}\/\d{2}$/
  assert.ok(datePattern.test(result.timeAndDate), 'Date should match YYYY/MM/DD format')
})

test('get_current_time returns time and date in correct order', () => {
  const result = getCurrentTime()
  const timeAndDatePattern = /^\d{2}:\d{2}:\d{2} \d{4}\/\d{2}\/\d{2}$/
  assert.ok(timeAndDatePattern.test(result.timeAndDate), 'Format should be HH:MM:SS YYYY/MM/DD')
})
...
test('get_current_time returns valid minute values (0-59)', () => {
  ...
  assert.ok(minutes >= 0 && minutes <= 59, 'Minutes should be between 0 and 59')
})
...
test('get_current_time returns valid second values (0-59)', () => {
  ...
  assert.ok(seconds >= 0 && seconds <= 59, 'Seconds should be between 0 and 59')
})
